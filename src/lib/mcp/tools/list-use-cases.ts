...
export default defineTool({
  ...
  inputSchema: {
    slug: z.string().optional().describe("Return a single use case by slug (e.g. scheduling-and-no-shows)."),
  },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
  handler: ({ slug }) => {
    const items = slug ? USE_CASES.filter((u) => u.slug === slug) : USE_CASES;
    if (slug && items.length === 0) {
      return { content: [{ type: "text", text: `No use case with slug "${slug}".` }], isError: true };
    }
